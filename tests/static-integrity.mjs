...
assert(
  appJs.includes('sports-chip--match')
    ...
    && appJs.includes('function sportsChipWithInstitution')
    && appJs.includes('function sportsInstitutionParen')
    && appJs.includes("Cégep de l'Outaouais")
    && appJs.includes('Cégep de Trois-Rivières')
    && appJs.includes('Cégep de Rimouski')
    && appJs.includes("'reçoit'")
    && appJs.includes("'chez'")
    && appJs.includes('sportsDisplaySideName')
    && appJs.includes('sportsChipOpponentLabel')
    && /const nick = sailing \? '' : String\(team\?\.nickname/.test(appJs)
    && /const nick = sailing \? '' : String\(game\?\.opponentNickname/.test(appJs)
