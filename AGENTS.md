...
- Catalogue interne : 8 056 notices (`news-archive.json`), mode `partial` (plafond public 120). Tranches d’âge : ~3 096 indexables (≤ 1 an), ~4 918 conservation (1–3 ans), 42 référence (> 3 ans). Liens vérifiés encore rares (`available` 48, `redirected` 377, `unknown` 7 631).
- Surface publique : **15 URLs** dans `sitemap-archives.xml` (hub `/archives/` + 14 publications). Conservation et référence : `noindex,follow`, hors sitemap. Canonicals = URL publique.
- Google (requête `site:le-radar.ca/archives/`) ne remonte pas ces pages ; aucun courriel Search Console / Bing / IndexNow. **D11** (clics de compte) bloque la vraie mesure d’impressions/clics.
- Horloge : catalogue public depuis ~2026-07-29 → revue **2026-09-09** (6 semaines) si D11 est fait. Filet : `tests/historical-catalog.mjs` refuse une page `noindex` dans le sitemap ou une page indexable oubliée.
- Interdit : `historical-catalog.config.json` `mode: full` tant que GSC/Bing n’ont pas 4–6 semaines de couverture + revue humaine.
