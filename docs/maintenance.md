...
- `update-sports.yml` — scores RSEQ/Spordle/voile : **matin · midi · fin de cours · 20 h · 22 h 30 · minuit+** (UTC mappé sur Amérique/Toronto ±1 h EST/EDT) + **week-end après-midi**. Après le fetch : `generate-seo.js --sports-only` pour que `/sports/` (HTML prérendu) suive `sports.json`. Abort si chute >50 % d’équipes ou majorité de ligues en panne ; sinon préserve le snapshot précédent par ligue. Hockey Spordle (`*.rseqhockey.com`) : challenge Cloudflare (y compris depuis le Worker `le-radar-hockey` et `curl_cffi`). Chaîne quand même : Worker → Python `hockey-spordle.py` → Node → snapshot. **Calendriers campus** (fonctionnent) : D2 M (UQO+UQAC) et D1 F (Stingers, Gaiters, McGill, Carabins, Rouge et Or). Push avec retry comme les autres bots. `sports.json` est en `paths-ignore` du Vérification (pas de Chromium à chaque refresh).
