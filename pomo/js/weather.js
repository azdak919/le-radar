...
(() => {
  ...
  const icon = (code, day) => {
    const isDay = !!day;
    let name = isDay ? 'overcast-day' : 'overcast-night';
    if (code === 0) name = isDay ? 'clear-day' : 'clear-night';
    else if ([1, 2].includes(code)) name = isDay ? 'partly-cloudy-day' : 'partly-cloudy-night';
    else if ([45, 48].includes(code)) name = isDay ? 'fog-day' : 'fog-night';
