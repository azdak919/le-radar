...
- Sur téléphone, les textes qui défilent s’arrêtaient définitivement au bout d’un aller-retour. Les lignes qui **ne changent jamais** — nom du poste dans le synthétiseur, établissement d’une pastille source — restaient donc tronquées pour le reste de la visite, sans même les trois points qui auraient signalé la coupure. Le défilement boucle maintenant : repos au début → aller → repos au bout → retour, indéfiniment. Le texte n’est toujours remplacé qu’au repos, jamais en pleine course.
- La carte CTA sports ne renouvelait jamais son accroche au doigt : la rotation était réservée aux surfaces à pointeur fin, seul endroit où le survol tenait lieu de mécanisme de pause (WCAG 2.2.2). Une fois son défilement terminé, la carte se figeait sur le même match jusqu’au rechargement. L’appui maintenu sur la carte suspend désormais la rotation — le mécanisme existe au doigt comme à la souris, la rotation aussi.
