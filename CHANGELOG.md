...
- Scores **en direct** sur la carte CTA : un match dans la fenêtre (coup d’envoi −15 min / +3 h) reste « En cours », affiche le score dès que RSEQ le colle, et n’est plus recouvert par un résultat d’hier. Le bot relit `GetGameDiffusion` et un cron `--live` toutes les 15 min pendant les plages de match ; le mât ressonde `sports.json` tant qu’un direct est à l’écran.
